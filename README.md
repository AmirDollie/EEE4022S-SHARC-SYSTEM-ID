...
implementation, carried in code comments at the relevant file - worth raising
with Malcolm/Robyn directly rather than treating as settled:

1. **Sign discrepancy** between the thesis and the 2013 JFM paper on the
   `+αφ`/`-αφ` coupling term in the plate boundary condition (2.13d) -
   centralised to one line in `dispersionFunction.m` if resolved.
2. **κ₋₁ vs κ₋₂ physical labelling** - the Bennetts j*=3-j swap convention is
...
3. **Validation still outstanding**: the β→∞ rigid limit, the γ→0
   ...
   been performed - see project notes for the current validation ladder.

---

## Pushing changes

From the repository root (`test_repo`):

```
git status
git add -A
git commit -m "Add README documenting Forward Model structure and function reference"
git push
```

`git status` first is worth checking every time - confirm only the files you
expect are staged (and that no stray `.asv` files have slipped past
